...
foreach(i = 1:length(gap_incompleted),
        .errorhandling = "pass",
        .combine = rbind) %do% {
          ...
          layer_gap_cleanroads <- GAP_layer_base %>% st_transform(5070) %>% st_make_valid()
          county_roads <- county_roads %>% st_transform(5070) %>% st_make_valid()
          ...
          # Keeping only relevant roads
          county_roads_relevant <-  filter(road_relevancy,relevant == T) %>% st_make_valid()
          ...
          layer_gap_cleanroads <- st_collection_extract(layer_gap_cleanroads, "POLYGON") %>% st_make_valid()
          
          #   GAP Cleaning: Cadastral                                                 ####
          #      Preparing Base Gap Layer that has been cleaned                       ####
          
          layer_gap_cleanroads_cleancadastral <- layer_gap_cleanroads 
          cadastral_exemption_shp <- county_cadastral %>% st_transform(5070) %>% st_make_valid()
          ...
          # Keeping only relevant cadastral datapoints
          cadastral_relevant <- filter(cadastral_relevancy,relevant == T) %>% st_make_valid()
