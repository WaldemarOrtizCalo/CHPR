...
#      NDVI Details                                                         ####
products <- mt_products()
bands <- mt_bands(product = "MYD13Q1")
dates <- mt_dates(product = "MYD13Q1", lat = 46.8797, lon = -110.3626) %>% 
  mutate(calendar_date = ymd(calendar_date)) %>% 
  filter(calendar_date >= ymd("2008-01-01"))
...
foreach (t = 1:nrow(dates),
         ...
         .errorhandling = "pass") %do% {
           ...
           foreach(i = 1:nrow(coord_grid),
             ...
             {
               # Data downloader
               montana_ndvi<- mt_subset(product = "MYD13Q1",
                                        lat = coord_grid[i,1],
                                        lon =  coord_grid[i,2],
           ...
           # Making a list of the raster tiles
           tiles <- list.files("1.Data/data_raw/NDVI_MODIS",
                               full.names = T,
                               pattern = "t\\d\\d\\d.tif") %>% 
             str_subset(pattern = date)
           ...
           # Exporting Raster
           writeRaster(x = final,
                       filename = paste0("1.Data/data_clean/NDVI_MODIS/NDVI_",
                                         date,
                                         "250m.tif"),
                       overwrite = T)
